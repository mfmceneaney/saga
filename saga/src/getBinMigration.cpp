...
void execute(const YAML::Node& node) {
    ...
    // OUTPATH
    std::string baseoutpath = "";//NOTE: This will be prepended to the default output path like so: `<baseoutpath><binscheme_name>_bin_migration.csv`.
    if (node["baseoutpath"]) {
    ...
    if (node["mc_suffix"]) {
        mc_suffix = node["mc_suffix"].as<std::string>();
    }
    std::cout << "INFO: mc_suffix: " << mc_suffix << std::endl;
    ...
    for (auto it = bincuts_map.begin(); it != bincuts_map.end(); ++it) {
        ...
        saga::bins::getBinMigration(
            ...
            mc_suffix //std::string                                                   mc_suffix = "_mc"
        );
    } // for (auto it = bincuts_map.begin(); it != bincuts_map.end(); ++it) {
