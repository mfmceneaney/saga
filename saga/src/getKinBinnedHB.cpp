...
void execute(const YAML::Node& node) {
    ...
    // Process arguments
    std::string   message_prefix  = "[getKinBinnedHB]: ";
    bool          verbose         = true;
    ...
    double alpha = saga::util::getYamlArg<double>(node, "alpha", 0.747, message_prefix, verbose); // Lambda decay asymmetry parameter from the PDG
    ...
    std::vector<std::string> categories_as_float; //NOTE: Only needed for argument of getKinBinnedHB() and createDataset()
