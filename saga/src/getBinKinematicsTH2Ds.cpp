...
void execute(const YAML::Node& node) {
    ...
    // Process arguments
    std::string   message_prefix  = "[getBinKinematicsTH2Ds]: ";
    bool          verbose         = true;
    ...
    bool save_pdfs = saga::util::getYamlArg<bool>(node, "save_pdfs", false, message_prefix, verbose);
    bool save_csvs = saga::util::getYamlArg<bool>(node, "save_csvs", false, message_prefix, verbose);
    ...
    std::vector<std::vector<std::string>> kinvars = saga::util::getYamlArg<std::vector<std::vector<std::string>>>(node, "kinvars", {}, message_prefix, verbose);
    // std::vector<std::string> kinvar_titles = saga::util::getYamlArg<std::vector<std::string>>(node, "kinvar_titles", {}, message_prefix, verbose); //NOTE: DEFAULT TO ACTUAL VARIABLE NAMES
    std::vector<std::vector<std::vector<double>>> kinvar_lims = saga::util::getYamlArg<std::vector<std::vector<std::vector<double>>>>(node, "kinvar_lims", {}, message_prefix, verbose);
    std::vector<std::vector<int>> kinvar_bins = saga::util::getYamlArg<std::vector<std::vector<int>>>(node, "kinvar_bins", {}, message_prefix, verbose);
