...
void execute(const YAML::Node& node) {
    ...
    // Process arguments
    std::string   message_prefix  = "[getBinKinematicsTH1Ds]: ";
    bool          verbose         = true;
    ...
    std::vector<std::vector<double>> kinvar_lims = saga::util::getYamlArg<std::vector<std::vector<double>>>(node, "kinvar_lims", {}, message_prefix, verbose);
    std::vector<int> kinvar_bins = saga::util::getYamlArg<std::vector<int>>(node, "kinvar_bins", {}, message_prefix, verbose);
    ...
    for (auto it = bincuts_map.begin(); it != bincuts_map.end(); ++it) {
        ...
        saga::bins::getBinKinematicsTH1Ds(
