...
void execute(const YAML::Node& node) {
    ...
    std::vector<std::vector<double>> kinvar_lims = saga::util::getYamlArg<std::vector<std::vector<double>>>(node, "kinvar_lims", {}, message_prefix, verbose, yamlargout);
    std::vector<int> kinvar_bins = saga::util::getYamlArg<std::vector<int>>(node, "kinvar_bins", {}, message_prefix, verbose, yamlargout);
