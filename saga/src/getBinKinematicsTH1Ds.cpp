...
void execute(const YAML::Node& node) {
    ...
    if (node["kinvars"]) {
        kinvars = node["kinvars"].as<std::vector<std::string>>();
    }
    std::cout << "INFO: kinvars: [ ";
    for (int idx=0; idx<kinvars.size(); idx++) {
        if (idx!=kinvars.size()-1) { std::cout << kinvars[idx]<<", "; }
        else { std::cout << kinvars[idx]; }
    }
    std::cout << " ]" << std::endl;
    ...
    if (node["kinvar_lims"]) {
        kinvar_lims = node["kinvar_lims"].as<std::vector<std::vector<double>>>();
    }
    std::cout << "INFO: kinvar_lims: [ ";
    for (int idx=0; idx<kinvar_lims.size(); idx++) {
        if (idx!=kinvar_lims.size()-1) { std::cout << "[ " << kinvar_lims[idx][0] << ", " << kinvar_lims[idx][1] << " ], "; }
        else { std::cout << "[ " << kinvar_lims[idx][0] << ", " << kinvar_lims[idx][1] << " ] "; }
    }
    std::cout << " ]" << std::endl;
    ...
    if (node["kinvar_bins"]) {
        kinvar_bins = node["kinvar_bins"].as<std::vector<int>>();
    }
    std::cout << "INFO: kinvar_bins: [ ";
    for (int idx=0; idx<kinvar_bins.size(); idx++) {
        if (idx!=kinvar_bins.size()-1) { std::cout << kinvar_bins[idx]<<", "; }
        else { std::cout << kinvar_bins[idx]; }
    }
    std::cout << " ]" << std::endl;
