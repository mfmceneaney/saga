...
def get_lims_coords(
    ...
    # Check node
    if (isinstance(node,dict) and nested_key in node \
        ...
        xlims = []
        if (isinstance(node_nested,dict) and nested_key in node_nested \
                ...
                for xidx in range(len(xlim[1:-1])):
                    el = [
                        ...
                        [
                            ...
                            ylims[yidx+1] if yidx<len(xlims)-1 else outer_ylims[-1]
                        ]
                    ]
...
def get_bin_centers(
    ...
    bin_centers = [
        [el[0].replace('(','').split(' && '),el[1].replace(')','').split(' && ')] for el in bin_centers
    ]
    bin_centers = [
        [
            [
                float(el_idx[0].split('>')[1]),
                float(el_idx[1].split('<')[1])
...
def plot_systematics(
    ...
    plt.hist(xbins, weights=yerr_syst, bins=nbins, alpha=0.5, label=syst_labels, stacked=stacked, log=log)
...
def plot_results(
    ...
    if yerr_syst is not None:
        ax1.errorbar(x_mean,y_mean,xerr=None,yerr=yerr_syst,
    ...
    if y_std is not None:
        ax1.fill_between(x_mean, np.add(y_mean,y_std), np.add(y_mean,-y_std), alpha=0.2, label='$\\pm1\\sigma$ Band', color=fill_color)

    # Plot results
    if yerr_mean is not None:
        ax1.errorbar(x_mean,y_mean,xerr=xerr_mean if plot_xerrors else None,yerr=yerr_mean,
                            ecolor=ecolor, elinewidth=elinewidth, capsize=capsize,
                            color=sg_colors[sgasym_idx], marker=marker, linestyle=linestyle,
    ...
    # Save plot data to csv
    delimiter = ","
    cols      = ["bin","count","x","y","xerr","yerr","xerrsyst","yerrsyst"] if not rescale \
...
def plot_results_array(
    ...
    if len(shape) not in (1,2):
        raise TypeError('`plot_projections()` : `graph_array` shape must have shape with len(shape) in (1,2) but shape = ',shape)
    ...
    # Save figure
    if 'lumi_ratio' in plot_results_kwargs_base and \
        plot_results_kwargs_base['lumi_ratio']>0.0:
